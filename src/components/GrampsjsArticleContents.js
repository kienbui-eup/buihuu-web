...
export class GrampsjsArticleContents extends LitElement {
  static get properties() {
    return {sections: {type: Array}, articleId: {type: String}}
  }
  ...
  static get styles() {
    return css`
      ...
      details {
        background: var(--md-sys-color-surface, #faf7ee);
        border: 1px solid var(--md-sys-color-outline-variant, #d6c7b3);
    ...
    return html`
      <details @keydown=${this._handleKeydown} @toggle=${this._fitPanel}>
        <summary>
          ...
          <span class="title">Xem nhanh mục lục</span>
    ...
    this.dispatchEvent(
      new CustomEvent('article-section:select', {
        ...
        detail: {
          key: section.key,
          label: section.label,
          offset: top + this.offsetHeight + 16,
        },
      })
    )
  }
  ...
  _fitPanel() {
    ...
    nav.style.maxHeight = `${Math.max(120, Math.min(400, available))}px`
  }
}
